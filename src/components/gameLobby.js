...
const GameLobby = (props) => {
  ...
  const sendChat = () => {
    ...
    chatSocket.send(JSON.stringify({client_chat: textArea.value}));
    textArea.value = "";
    textArea.focus();
  }

  useEffect(() => {
    if(!socket && !chatSocket && slug && userContext.access){
      const url = `${process.env.REACT_APP_SOCKET_ENDPOINT}gameinstance/${slug}?player=${cookies.user}&token=${userContext.access}`;
      const chatUrl = `${process.env.REACT_APP_SOCKET_ENDPOINT}chat/${slug}?player=${cookies.user}&token=${userContext.access}`;
      let newSocket = new WebSocket(url);
      let chatSocket = new WebSocket(chatUrl);
      setSocket(newSocket);
      setChatSocket(chatSocket);
    }
    if(!firstRequestDone && userContext.access && socket){
      socket.addEventListener('message', socketListener);
      chatSocket.addEventListener('message', chatListener);
  ...
  return (
        ...
        <div className="transition-all duration-300 flex flex-col w-full h-auto bg-manatee rounded-b-lg">
          ...
          {error && <h1 className="mt-4 text-lg text-red-600">{error}</h1>}
            ...
            <form id="gameDetailsForm" className="flex flex-col h-full w-full justify-start items-center pt-2">
              <label className="text-aliceBlue mr-2" htmlFor="instanceInput">Invite Code</label>
    ...
    <div className="transition-all ease-out duration-500 flex flex-col h-full w-3/12 items-center border border-gray-400">
      ...
      <div id="chatBox" className="flex-col h-5/6 w-full bg-gray-200 rounded rounded-b-md overflow-auto">
        {messages.map((m,i) => {
          let color = i % 2 === 0 ? "bg-manatee" : "bg-gray-300";
          let className = `flex-col w-auto h-auto place-items-start m-2 p-2 rounded-md ${color}`;
        return <div className={className}>
          <h2 className="flex text-md justify-start text-left font-medium">{m.username}</h2>
          <h2 className="flex text-md justify-start text-left">{m.chat_message}</h2>
        </div>
      })}
      </div>
      <div className="w-full h-1/6 py-2">
        <textarea placeholder="Enter Message" id="chatTextArea" className="bg-white w-full h-full" onKeyPress={(e) => {if(!e.shiftKey && e.key === "Enter"){sendChat(); e.preventDefault();}}}></textarea>
      </div>
      <button className=" transition-all duration-300 bg-gray-400 w-11/12 h-8 my-4 rounded-md self-center mr-2 hover:bg-gray-300"
        onClick={() => {sendChat();}}>
          Send
      </button>
    </div>  
